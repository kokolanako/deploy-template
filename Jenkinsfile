    ...
    post{
        failure {
            emailext   subject: "ERROR CI: Project name -> ${env.JOB_NAME}", to: "Polina.Mrachkovskaya@kisters.de", recipientProviders: [developers()],
                    body: "<b>Example</b><br>\n<br>Project: ${env.JOB_NAME} <br>Build Number: ${env.BUILD_NUMBER} <br> URL de build: ${env.BUILD_URL}", cc: '', charset: 'UTF-8', from: '', mimeType: 'text/html', replyTo: '', subject: "ERROR CI: Project name -> ${env.JOB_NAME}";
        }
    }
//    post{
//        always{
